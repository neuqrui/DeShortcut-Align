...
def resolve_chat_template_source(value: Optional[str]) -> Optional[str]:
    """Return Jinja source for a chat template.

    Hydra often passes ``model.custom_chat_template=/path/to/foo.jinja``. If that string is assigned
    directly to ``tokenizer.chat_template``, Hugging Face will treat the *path text* as the template
    body; ``apply_chat_template`` then renders to that path string (breaking inference).

    This helper loads the file when ``value`` points to an existing ``.jinja`` / ``.j2`` file.

    Also fixes checkpoints where ``chat_template.jinja`` mistakenly contains only another template path
    as plain text (no ``{%`` / ``{{``): follow that path and load the real template.
    """
    if value is None or not isinstance(value, str):
        return value
    s = os.path.expanduser(value.strip().strip('"').strip("'"))
    if not s:
        return value
    if os.path.isfile(s) and s.lower().endswith((".jinja", ".j2")):
        with open(s, encoding="utf-8") as f:
            body = f.read()
        inner = os.path.expanduser(body.strip().strip('"').strip("'"))
        if (
            "{%" not in body
            and "{{" not in body
            and os.path.isfile(inner)
            and inner.lower().endswith((".jinja", ".j2"))
        ):
            with open(inner, encoding="utf-8") as f2:
                return f2.read()
        return body
    return value

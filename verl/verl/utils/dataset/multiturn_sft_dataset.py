...
class MultiTurnSFTDataset(Dataset):
    ...
    @staticmethod
    def _normalize_data_source_value(raw: Any) -> str | None:
        if raw is None:
            return None
        try:
    ...
    def _process_single_message(
        ...
        Tokenizes ``full_message[: index + 1]`` and takes the delta vs the previous
        prefix. This is required for Qwen3-style templates that only re-embed
        ``<think>`` when the assistant turn is after ``last_query_index``; applying
        the template to an assistant-only message strips thinking and would train
        the model to answer without ``<think>``.

        Args:
            index: turn index in the conversation
            message: A single message dictionary
            full_message: Full conversation messages
        ...
        # Cumulative apply so chat templates that depend on full history (e.g. Qwen3
        # last_query_index / <think> re-embed) see the correct context.
        cur_inputs = apply_chat_template(
            processor,
        ...
        cur_inputs = dict(cur_inputs)
        cur_ids = cur_inputs.pop("input_ids")[0]
        cur_attn = cur_inputs.pop("attention_mask")[0]

        if index == 0:
            input_ids = cur_ids
            attention_mask = cur_attn
            mm_inputs = cur_inputs
        else:
            ...
            prev_inputs = dict(prev_inputs)
            prev_ids = prev_inputs.pop("input_ids")[0]
            prev_len = prev_ids.shape[0]
            if cur_ids.shape[0] < prev_len or not torch.equal(cur_ids[:prev_len], prev_ids):
                raise AssertionError(
                    "Cumulative chat-template prefix mismatch when slicing turn "
                    f"{index}. Qwen3/thinking templates must be applied on the full "
                    "prefix, not on a single assistant message. "
                    f"prev_len={prev_len}, cur_len={cur_ids.shape[0]}."
                )
            input_ids = cur_ids[prev_len:]
            attention_mask = cur_attn[prev_len:]
            # Multimodal tensors are already represented in the full prefix; avoid
            # double-counting by not attaching prev-turn vision features again.
            mm_inputs = {}

        if message["role"] == "assistant":
            loss_mask = torch.ones_like(attention_mask)
            # mask out generation prompt if assistant message
            gen_len = min(len(self.generation_prompt), loss_mask.shape[0])
            loss_mask[:gen_len] = 0
        else:
            loss_mask = torch.zeros_like(attention_mask)

        return input_ids, loss_mask, attention_mask, mm_inputs
    ...
    def __getitem__(self, item):
        ...
        # 1. tokenize each message (cumulative prefix + delta; keeps Qwen3 <think>)
        ...
        if self.pad_mode == DatasetPadMode.RIGHT:
            ...
            if self.data_source_key in row_dict:
                ds = self._normalize_data_source_value(row_dict.get(self.data_source_key))
                if ds is not None:
                    res["data_source"] = ds
            return res

...
class SFTTrainer:
    ...
    def _build_dataset(self):
        ...
        # CCR on harmful rows only: benign contrastive samples must not be trained toward refusal.
        ccr_cfg = config.model.get("ccr", {})
        ccr_only_harmful = bool(ccr_cfg.get("only_harmful", False))
        if ccr_only_harmful:
            data_source_key = str(ccr_cfg.get("data_source_key", "data_source"))

            def _assert_data_source_ok(ds, split_name: str):
                if ds is None:
                    return
                if not hasattr(ds, "dataframe"):
                    raise AssertionError(
                        f"`model.ccr.only_harmful=true` requires `{data_source_key}` checks, "
                        f"but {split_name} dataset has no `dataframe` attribute."
                    )
                df = ds.dataframe
                if data_source_key not in df.columns:
                    raise AssertionError(
                        f"`model.ccr.only_harmful=true` requires `{data_source_key}` column in "
                ...
                label_set = set(values.unique().tolist())
                missing = {"harmful", "benign"} - label_set
                if missing:
                    raise AssertionError(
                        f"`model.ccr.only_harmful=true` requires `{data_source_key}` in {split_name} "
                        f"to contain both `harmful` and `benign`, got: {sorted(label_set)}"
                    )

            _assert_data_source_ok(train_dataset, "train")
            _assert_data_source_ok(val_dataset, "val")
    ...
    def fit(self):
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            epoch_meta = {**meta_info, "sft_epoch_1based": int(epoch + 1)}
            ...
            for step_in_epoch, data in enumerate(
                ...
                data = tu.get_tensordict(tensor_dict=data, non_tensor_dict=epoch_meta)
                ...
                if self.engine.is_mp_src_rank_with_outputs():
                    ...
                    # Convert extra metrics (e.g. qm aux) to scalar values for trackers.
                    # allgather_dict_into_dict may return per-dp lists here.
                    for k in list(metrics.keys()):
                        ...
                        if isinstance(v, list):
                            if len(v) > 0:
                                try:
                                    scalar = torch.tensor(v, dtype=torch.float32).mean().item()
                        ...
                        if scalar is not None:
                            metrics[f"train/{k}"] = float(scalar)
                ...
                if is_last_step and self.val_dataloader is not None or (self.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        val_data = tu.get_tensordict(tensor_dict=val_data, non_tensor_dict=epoch_meta)

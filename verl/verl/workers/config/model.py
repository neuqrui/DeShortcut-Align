...
from verl.utils.chat_template import resolve_chat_template_source
...
class HFModelConfig(BaseConfig):
    ...
    # Counterfactual Consistency Regularization (CCR) for SFT.
    # See FSDPEngineWithLMHead.forward_step / losses.sft_loss.
    ccr: dict[str, Any] = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        if self.custom_chat_template is not None:
            _tpl = resolve_chat_template_source(str(self.custom_chat_template))
            if self.processor is not None:
                self.processor.chat_template = _tpl
            else:
                self.tokenizer.chat_template = _tpl

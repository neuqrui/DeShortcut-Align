...
class FSDPEngineWithLMHead(FSDPEngine):
    ...
    def _prepare_padded_inputs_fmt_blinded(self, micro_batch: TensorDict, fmt_token_ids: list[int]):
        """Padded inputs with attention zeroed on formatting tokens (template-ablated state X_cf)."""
        pad_mode = tu.get_non_tensor_data(data=micro_batch, key="pad_mode", default=DatasetPadMode.NO_PADDING)
        assert pad_mode == DatasetPadMode.NO_PADDING, f"pad_mode {pad_mode} not supported for CCR"
        input_ids = micro_batch["input_ids"]
        position_ids = micro_batch["position_ids"]
        loss_mask = micro_batch["loss_mask"]
        temperature = micro_batch["temperature"]
        ...
        input_ids_rmpad_rolled = torch.roll(input_ids.values(), shifts=-1, dims=0)
        output_args = {"input_ids_rmpad_rolled": input_ids_rmpad_rolled, "temperature": temperature}
        ...
        attention_mask_list = [torch.ones_like(t, dtype=torch.int32) for t in loss_mask]
        attention_mask_nested = torch.nested.as_nested_tensor(attention_mask_list, layout=torch.jagged)
        attention_mask = torch.nested.to_padded_tensor(
            attention_mask_nested, padding=0, output_size=(batch_size, max_seq_len)
        )

        if fmt_token_ids:
            tags = torch.tensor(fmt_token_ids, device=input_ids_padded.device, dtype=input_ids_padded.dtype)
            hit = (input_ids_padded.unsqueeze(-1) == tags.view(1, 1, -1)).any(dim=-1)
            attention_mask = attention_mask * (~hit).to(attention_mask.dtype)
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        ccr_cfg = getattr(self.model_config, "ccr", None) or {}
        try:
            from omegaconf import OmegaConf

            if OmegaConf.is_config(ccr_cfg):
                ccr_cfg = OmegaConf.to_container(ccr_cfg, resolve=True)
        except ImportError:
            pass
        if not isinstance(ccr_cfg, dict):
            ccr_cfg = dict(ccr_cfg) if ccr_cfg is not None else {}

        fmt_ids = list(ccr_cfg.get("fmt_token_ids") or [])
        do_ccr = (
            not forward_only
            and loss_function is not None
            and bool(ccr_cfg.get("enable", False))
            and float(ccr_cfg.get("alpha", 0.0)) > 0.0
            and len(fmt_ids) > 0
        )
        if do_ccr:
            use_rmpad = tu.get_non_tensor_data(data=micro_batch, key="use_remove_padding", default=True)
            use_fused = tu.get_non_tensor_data(data=micro_batch, key="use_fused_kernels", default=False)
            mm = extract_multi_modal_inputs(micro_batch.get("multi_modal_inputs", []))
            if use_fused or self.use_ulysses_sp or mm or not use_rmpad:
                logger.warning(
                    "CCR skipped (fused=%s ulysses=%s multimodal=%s use_remove_padding=%s)",
                    use_fused,
                    self.use_ulysses_sp,
                    bool(mm),
                    use_rmpad,
        ...
        # Defer CCR until model.ccr.start_epoch (1-based, matches tqdm "Epoch k/total").
        sft_epoch_1based = tu.get_non_tensor_data(data=micro_batch, key="sft_epoch_1based", default=None)
        if do_ccr and sft_epoch_1based is not None:
            ccr_start = int(ccr_cfg.get("start_epoch", 1))
            if int(sft_epoch_1based) < ccr_start:
                do_ccr = False
        ...
        with torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            model_output_ccr = None
            ccr_alpha = 0.0
            ccr_loss_type = "ce"
            ccr_kl_penalty = "kl"
            ccr_only_harmful = False
            ccr_data_source_key = "data_source"

            if do_ccr:
                try:
                    mi_cf, oa_cf = self._prepare_padded_inputs_fmt_blinded(micro_batch, fmt_ids)
                    raw_cf = self.module(**mi_cf, use_cache=False)
                    micro_batch_cf = micro_batch.clone(recurse=False)
                    tu.assign_non_tensor_data(micro_batch_cf, "use_remove_padding", False)
                    model_output_ccr = self.prepare_model_outputs(
                        output=raw_cf,
                        output_args=oa_cf,
                        micro_batch=micro_batch_cf,
                        logits_processor_func=loss_function,
                    )
                    ccr_alpha = float(ccr_cfg.get("alpha", 1.0))
                    ccr_loss_type = str(ccr_cfg.get("loss_type", "ce")).lower()
                    ccr_kl_penalty = str(ccr_cfg.get("kl_penalty", "kl"))
                    ccr_only_harmful = bool(ccr_cfg.get("only_harmful", False))
                    ccr_data_source_key = str(ccr_cfg.get("data_source_key", "data_source"))
                except (NotImplementedError, RuntimeError) as e:
                    logger.warning("CCR skipped: %s", e)
            ...
            if loss_function is not None:
                loss, metrics = loss_function(
                    ...
                    model_output_ccr=model_output_ccr,
                    ccr_alpha=ccr_alpha,
                    ccr_loss_type=ccr_loss_type,
                    ccr_kl_penalty=ccr_kl_penalty,
                    ccr_only_harmful=ccr_only_harmful,
                    ccr_data_source_key=ccr_data_source_key,
                )
            else:

...
def prepare_micro_batches(
    ...
    if use_dynamic_bsz:
        ...
        if not (total_data_size % (force_group_size * micro_batch_size_per_gpu) == 0):
            print(f"total_data_size {total_data_size} is not divisible by {force_group_size} * {micro_batch_size_per_gpu}")

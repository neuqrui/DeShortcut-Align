...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def _postprocess_output(self, output, *, global_token_num, delta_time, forward_only, images_seqlens):
        ...
        # all_gather_object returns a list of per-rank scalars. Loggers expect one float per step (for example cf_loss).
        if dp_group is not None:
            for k, v in list(final_metrics.items()):
                if k.startswith("mtp_losses"):
                    continue
                if isinstance(v, list) and v:
                    try:
                        final_metrics[k] = sum(float(x) for x in v) / len(v)

...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        lambda_ccr = data.meta_info.get("lambda_ccr", 0.0)
        fmt_token_ids = data.meta_info.get("fmt_token_ids", [])
        ccr_loss_type = data.meta_info.get("ccr_loss_type", "ce_loss")
        ccr_kl_penalty = data.meta_info.get("ccr_kl_penalty", "low_var_kl")
        ccr_ce_mode = str(data.meta_info.get("ccr_ce_mode", "hard")).lower()
        ccr_enable = data.meta_info.get("ccr_enable", False)
        rho_max = data.meta_info.get("rho_max", 10.0)
        tau = float(data.meta_info.get("tau", 0.001))
        ...
        if "ccr_gate" in data.batch.keys():
            select_keys.append("ccr_gate")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # CCR: attention-blind formatting tokens, then match the verified rollout on X_cf.
                    # L_cf = G * (-log pi(y | X_cf)) when loss_type=ce_loss and ce_mode=hard.
                    # lambda_dyn = lambda * rho_max / r when |L_pg| >= tau and r > rho_max, else lambda.
                    ccr_gate = model_inputs.get("ccr_gate", None)

                    cf_loss_val = 0.0
                    lambda_dyn_val = 0.0
                    if ccr_enable and ccr_gate is not None and len(fmt_token_ids) > 0:
                        has_valid_samples_on_this_gpu = ccr_gate.any().item()
                        if torch.distributed.get_rank() == 0 and has_valid_samples_on_this_gpu:
                            print(f"[CCR] Computing counterfactual consistency (lambda={lambda_ccr})")

                        # Clone the full batch so every rank joins the second forward.
                        masked_inputs = {k: v.clone() if isinstance(v, torch.Tensor) else v for k, v in
                                        model_inputs.items()}
                        masked_attention_mask = masked_inputs["attention_mask"]
                        masked_input_ids = masked_inputs["input_ids"]

                        ccr_gate_2d = ccr_gate.unsqueeze(1)

                        # Blind formatting tokens S_fmt for gated samples.
                        for fmt_id in fmt_token_ids:
                            fmt_mask = (masked_input_ids == fmt_id)
                            valid_fmt_mask = fmt_mask & ccr_gate_2d
                            masked_attention_mask[valid_fmt_mask] = 0

                        masked_outputs = self._forward_micro_batch(
                            masked_inputs,
                            temperature=temperature,
                            calculate_entropy=False
                        )

                        masked_log_probs = masked_outputs["log_probs"]
                        gated_response_mask = masked_inputs["response_mask"]
                        original_log_probs = log_prob.detach()

                        valid_mask = gated_response_mask * ccr_gate_2d.float()
                        valid_tokens = valid_mask.sum()

                        if has_valid_samples_on_this_gpu:
                            if ccr_loss_type == "kl":
                                consistency_matrix = kl_penalty(
                                    logprob=masked_log_probs,
                                    ref_logprob=original_log_probs,
                                    kl_penalty=ccr_kl_penalty,
                                )
                            elif ccr_loss_type == "ce_loss":
                                if ccr_ce_mode == "hard":
                                    # Hard CE on the sampled tokens: L_cf = -log pi(y | X_cf).
                                    consistency_matrix = -masked_log_probs
                                else:
                                    teacher_prob = torch.exp(original_log_probs).detach()
                                    consistency_matrix = -teacher_prob * masked_log_probs
                            else:
                                consistency_matrix = (masked_log_probs - original_log_probs) ** 2

                            cf_loss = agg_loss(
                                loss_mat=consistency_matrix,
                                loss_mask=valid_mask,
                                loss_agg_mode=loss_agg_mode
                            )
                            if lambda_ccr == 0.0:
                                policy_loss = policy_loss + 0.0 * cf_loss
                                cf_loss_val = cf_loss.detach().item()
                                lambda_dyn_val = 0.0
                            else:
                                scaled_cf = (lambda_ccr * cf_loss).detach().abs().item()
                                current_pg_val = pg_loss.detach().abs().item()

                                if current_pg_val < tau:
                                    lambda_dyn = lambda_ccr
                                else:
                                    ratio = scaled_cf / (current_pg_val + 1e-8)
                                    if ratio > rho_max:
                                        lambda_dyn = lambda_ccr * (rho_max / ratio)
                                    else:
                                        lambda_dyn = lambda_ccr

                                policy_loss = policy_loss + lambda_dyn * cf_loss
                                print(
                                    f"[CCR] enable={ccr_enable} has_valid={has_valid_samples_on_this_gpu} "
                                    f"lambda={lambda_ccr:.6f} lambda_dyn={float(lambda_dyn):.6f} "
                                    f"cf_loss={float(cf_loss.detach()):.6f} pg_loss={float(pg_loss.detach()):.6f}"
                                )
                                cf_loss_val = cf_loss.detach().item()
                                lambda_dyn_val = lambda_dyn
                        else:
                            nll = -(masked_log_probs * gated_response_mask)
                            dummy_loss = nll.sum() / (gated_response_mask.sum() + 1e-8)
                            policy_loss = policy_loss + 0.0 * dummy_loss

                            cf_loss_val = 0.0
                            lambda_dyn_val = 0.0

                    if ccr_enable:
                        micro_batch_metrics["actor/cf_loss"] = cf_loss_val
                        micro_batch_metrics["actor/lambda_dyn"] = lambda_dyn_val
